...
public class SwipeScroller extends Animation  implements NavigationAnimator {
	...
	/**
	 * The rank of the current focused item.
	 */
	...
	public void setFocusWidget(int number) {
		slider.getWidget(currentItem).setFocus(false);
    ...
    private void scroll(int delta, int duration) {
    	positionBeforeAnimation = movingWidget.getAbsoluteLeft();
    	this.delta = delta;
	...
	private void onMove(int newXPosition) {
		scroll(newXPosition - movingRelativePosition, QUICK);
	...
	private void onEnd(int newXPos) {
		...
		setFocusWidget(currentItem);
		
		if (previousItem != currentItem)
    		slider.getWidget(currentItem).ignite();
	}
